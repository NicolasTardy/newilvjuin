...
# Mapping Excel LIB_FAMILLE → rayonILV (pour import Excel admin)
EXCEL_FAMILLE_TO_RAYON_ILV = {
    "CUISINIÈRE":                "CUISINIERE",
    "FOUR":                      "FOUR",
    "TABLE DE CUISSON":          "TABLE DE CUISSON",
    "RÉFRIGÉRATEUR":             "REFRIGERATEUR",
    "REF US (SIDE/SIDE)":        "REFRIGERATEUR US",
    "COMBINÉ":                   "COMBINE",
    ...
    "PETIT DÉJEUNER":            "EXPRESSO",
    "FMO":                       "FMO (Four Micro-Ondes)",
    "HOTTE":                     "HOTTE",
    "TV":                        "TV",
    "CLIM":                      "CLIM",
    "SALON CUIR":                "SIEGE",
    "SALON TISSU":               "SIEGE",
    "CAC ADULTE":                "CAC (Canape/Convertible)",
    "CAC JUNIOR":                "CAC (Canape/Convertible)",
    "ENSEMBLE MATELAS ET SOMMIER": "LITERIE (Meuble)",
    "SOMMIERS":                  "LITERIE (Meuble)",
    "MATELAS":                   "LITERIE (Meuble)",
    "BIBLIOTHEQUE (SEJOUR)":     "SEJOUR",
    "ENFILADE":                  "SEJOUR",
    "MEUBLE TV (SEJOUR)":        "SEJOUR",
    "TABLE BASSE":               "SEJOUR",
    "VITRINE (SEJOUR)":          "SEJOUR",
    "TABLE (SEJOUR)":            "SEJOUR",
    "CUISINE KIT":               "CUISINE (meuble)",
    "CUISINE MONTEE":            "CUISINE (meuble)",
    "ARMOIRE DRESSING UNITAIRE": "RANGEMENT",
    "ENSEMBLE DE SALLE DE BAIN": "RANGEMENT",
}
...
# Simulateur tarifs services
# [univers, famille, prixMin, prixMax, gar1, gar2, gar3,
...
def lookup_warranty_all(rayon_ilv, prix):
    """Retourne garanties + tarifs services pour ce rayon et ce prix.
    ...
    for d in SIM_DONNEES:
        if d["famille"] == rayon_ilv and d["prixMin"] <= prix <= d["prixMax"]:
            return {
                ...
                "livraison": d["livDom"] if d["livDom"] > 0 else 0,
                "livPdP": d["livPdP"] if d["livPdP"] > 0 else 0,
                "livDom": d["livDom"] if d["livDom"] > 0 else 0,
                "montage": d["montage"] if d["montage"] > 0 else 0,
                "livEnsPdP": d["livEnsPdP"] if d["livEnsPdP"] > 0 else 0,
                "livEnsDom": d["livEnsDom"] if d["livEnsDom"] > 0 else 0,
            }
    return {"gar1": 0, "gar2": 0, "gar3": 0, "livraison": 0,
            "livPdP": 0, "livDom": 0, "montage": 0, "livEnsPdP": 0, "livEnsDom": 0}
